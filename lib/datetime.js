...
class DateTime {
    constructor(year, month, day, hour, minute, second, millisecond, kind = DateTimeKind.Unspecified) {
        let ticks = 0;
     ...
     */
    get dayOfWeek() {
        return (Math.floor(this.internalTicks / DateTime.TicksPerDay + 1) % 7);
    }
    /**
    ...
    static TimeToTicks(hour, minute, second) {
        ...
        if (hour >= 0 && hour < 24 && minute >= 0 && minute < 60 && second >= 0 && second < 60) {
            return (timespan_1.default.TimeToTicks(hour, minute, second));
        }
    ...
    subtract(value) {
        return new timespan_1.default(this.internalTicks - value.internalTicks);
    }
    subtractTime(value) {
        const ticks = this.internalTicks;
...
DateTime.formatChars = ['h', 'H', 'M', 'm', 't', 's', 'y', 'd', '*'];
